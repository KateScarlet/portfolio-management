...
export default function SummaryDashboard({ summary, colorScheme, onClose }: Props) {
  ...
  const pnl = summary.principal > 0 ? ((summary.total - summary.principal) / summary.principal) * 100 : 0
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <p className="text-lg font-semibold">¥{summary.total.toLocaleString("zh-CN", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
          </div>
          <div>
            <p className="text-xs text-[#6C757D]">总投入</p>
            <p className="text-lg font-semibold">¥{summary.principal.toLocaleString("zh-CN", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
          </div>
          <div>
            <p className="text-xs text-[#6C757D]">总收益</p>
            <p className={`text-lg font-semibold ${pnlColor(pnl)}`}>{pnl > 0 ? "+" : ""}{pnl.toFixed(1)}%</p>
          </div>
        </div>
          ...
          {(["stocks", "bonds", "cash", "commodities"] as AssetId[]).map((id) => {
            ...
            return (
              <div key={id} className="flex items-center gap-2 text-sm">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ASSET_DEFINITIONS[id].color }} />
                ...
                <div className="flex-1 h-2 bg-[#F8F9FA] rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: ASSET_DEFINITIONS[id].color }} />
                </div>
                <span className="w-12 text-right text-xs">{pct.toFixed(1)}%</span>
                <span className="w-20 text-right text-xs text-[#6C757D]">¥{val.toLocaleString("zh-CN", { maximumFractionDigits: 0 })}</span>
              </div>
            )
          })}
        </div>
            ...
            return (
              <div key={p.id} className="border border-[#E9ECEF] rounded p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm">¥{p.total.toLocaleString("zh-CN", { maximumFractionDigits: 0 })}</span>
                </div>
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-[#6C757D]">投入 ¥{p.principal.toLocaleString("zh-CN", { maximumFractionDigits: 0 })}</span>
                  <span className={`text-xs ${pnlColor(pPnl)}`}>{pPnl > 0 ? "+" : ""}{pPnl.toFixed(1)}%</span>
                </div>
              </div>
            )
          })}
