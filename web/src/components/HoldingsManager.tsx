...
import { AssetId, ASSET_DEFINITIONS, Holding } from '../types';
...
export default function HoldingsManager({ holdings, total, onAddHolding, onUpdateHolding, onRemoveHolding, onSaveRecord }: HoldingsManagerProps) {
    ...
    if (deductFromCash && addedCost > 0) {
      ...
      if (cashHolding) {
          ...
          onUpdateHolding(cashHolding.id, {
              ...
              cost: (cashHolding.cost || cashHolding.value) - addedCost > 0 ? (cashHolding.cost || cashHolding.value) - addedCost : remainingValue
          });
      }
    }
  ...
  const syncPrice = async (h: Holding) => {
    if (!h.symbol) return;
    try {
      const res = await fetch(`/api/price/${h.symbol}?targetCurrency=CNY`);
      const data = await res.json();
      if (data && data.price) {
  ...
  const confirmSell = () => {
    ...
    if (h.shares && h.shares > 0 && h.price) {
        ...
        if (remainingShares === 0) {
            ...
            const remainingCost = h.cost ? (h.cost / h.shares) * remainingShares : 0;
            onUpdateHolding(h.id, { 
        ...
        if (remainingValue === 0) {
            ...
            const remainingCost = h.cost ? (h.cost / h.value) * remainingValue : 0;
            onUpdateHolding(h.id, {
                value: remainingValue,
                cost: remainingCost
            });
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E9ECEF] shadow-sm flex flex-col overflow-hidden">
      <div className="p-6 border-b border-[#F1F3F5] flex justify-between items-center bg-white flex-wrap gap-4">
        <h3 className="text-lg font-medium text-[#1A1A1A]">资产明细 (Holdings)</h3>
         ...
         <div className="p-6 bg-[#F8F9FA] border-b border-[#E9ECEF] flex flex-col gap-4">
            ...
            <div className="flex gap-4 mb-2">
               <label className="flex items-center gap-2 text-sm text-[#495057]">
                 <input type="radio" checked={!isManual} onChange={() => setIsManual(false)} className="text-[#1A1A1A] focus:ring-[#1A1A1A]" />
                 自动获取价格 (支持美股/A股/ETF)
               </label>
               <label className="flex items-center gap-2 text-sm text-[#495057]">
                 <input type="radio" checked={isManual} onChange={() => setIsManual(true)} className="text-[#1A1A1A] focus:ring-[#1A1A1A]" />
                 手动录入价值
               </label>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4">
               <div className="flex flex-col gap-1">
                 <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">分类</label>
                 <select 
                   value={newAssetId} 
                 ...
                 >
                   {Object.keys(ASSET_DEFINITIONS).map(key => (
                     <option key={key} value={key}>{ASSET_DEFINITIONS[key as AssetId].name}</option>
                   ))}
                 </select>
               </div>
               <div className="flex flex-col gap-1">
                 <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">购买/录入日期</label>
                 <input 
                   type="date" 
                   value={newDate}
               ...
               {!isManual ? (
                 <>
                   <div className="flex flex-col gap-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">代码</label>
                     <input 
                       type="text" 
                       placeholder="如 VTI 或 510300" 
                       value={newSymbol}
                   ...
                   <div className="flex flex-col gap-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">名称 (选填)</label>
                     <input 
                       type="text" 
                       placeholder="如: 沪深300" 
                       value={newName}
                   ...
                   <div className="flex flex-col gap-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">买入单价 (选填, 货币)</label>
                   ...
                   <div className="flex flex-col gap-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">份额 (股份)</label>
                     <input 
                       type="number" 
                       placeholder="0" 
                       value={newShares}
                 ...
                 <>
                   ...
                   <div className="flex flex-col gap-1 md:col-span-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">总成本 (选填)</label>
                     <input 
                       type="number" 
                       placeholder="投入本金" 
                       value={newCost}
                   ...
                   <div className="flex flex-col gap-1 md:col-span-1">
                     <label className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold">当前总市值</label>
                     <input 
                       type="number" 
                       placeholder="最新估值" 
                       value={newValue}
               ...
               <div className="flex flex-col justify-end gap-2">
                 <label className="flex items-center gap-2 cursor-pointer pb-1 h-[21px]">
                   ...
                   />
                   <span className="text-[10px] uppercase tracking-widest text-[#495057] font-bold">从现金扣除本金</span>
                 </label>
                 <button 
                   onClick={handleAdd}
                   disabled={isFetching}
            ...
            {holdings.length === 0 ? (
              ...
              return (
                <React.Fragment key={h.id}>
                  <tr className="hover:bg-[#F8F9FA] transition-colors group cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : h.id)}>
                    <td className="px-6 py-5 flex items-center gap-3">
                      ...
                      >
                        {h.assetId === 'stocks' ? 'STK' : h.assetId === 'bonds' ? 'BND' : h.assetId === 'gold' ? 'CMD' : 'CSH'}
                      </div>
                      <div>
                      ...
                      {h.symbol ? (
                         <div>
                           <p className="text-sm font-mono flex items-center gap-2">
                             {h.symbol}
                             {h.lots && h.lots.length > 0 && <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{h.lots.length} 笔</span>}
                           </p>
                           <p className="text-[10px] text-[#ADB5BD] truncate max-w-[150px]" title={h.name}>{h.name}</p>
                         </div>
                      ) : (
                      ...
                      {h.cost && h.cost > 0 ? (
                        <div>
                          ...
                          {(() => {
                            ...
                            return (
                              <p className={`text-[10px] ${isPositive ? 'text-emerald-600' : 'text-orange-600'}`}>
                    ...
                    <td className="px-6 py-5 text-right font-medium text-sm font-mono" onClick={e => e.stopPropagation()}>
                      {editingValueId === h.id ? (
                    ...
                    <tr className="bg-[#F8F9FA]">
                      <td colSpan={6} className="px-6 py-4">
                        <div className="pl-12 border-l-2 border-[#DEE2E6] ml-4 space-y-2">
                          <h5 className="text-[10px] uppercase tracking-widest text-[#ADB5BD] font-bold mb-2">购买/录入记录</h5>
                          {h.lots.map(lot => (
                            <div key={lot.id} className="flex justify-between items-center text-xs text-[#495057] font-mono border-b border-[#E9ECEF] last:border-0 pb-2 last:pb-0">
                              <span>{new Date(lot.date).toLocaleDateString()}</span>
                              <div className="flex gap-8 text-right">
                                {h.symbol ? (
                                  <>
                                    <span className="w-24">单价: {formatCurrency(lot.costPrice || 0)}</span>
                                    <span className="w-24">数量: {lot.shares}</span>
                                    <span className="w-24 font-medium text-[#1A1A1A]">成本: {formatCurrency(lot.cost || 0)}</span>
                                  </>
                                ) : (
                                  <span className="w-24 font-medium text-[#1A1A1A]">新增价值: {formatCurrency(lot.valueAdded || lot.cost || 0)}</span>
                                )}
                              </div>
                            </div>
                          ))}
        ...
        <div className="fixed inset-0 bg-[#1A1A1A]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm text-[#6C757D] mt-1">{sellingHolding.name || sellingHolding.symbol}</p>
            </div>
