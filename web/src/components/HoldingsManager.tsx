        ...
        if (isManual) {
            ...
            try {
                const res = await fetch(
                    `/api/price/${newSymbol}?targetCurrency=CNY`,
                );
                ...
                if (data && data.price) {
                    ...
                    let finalCostPrice =
                        isNaN(cPrice) || cPrice <= 0 ? data.price : cPrice;
                    ...
                    onAddHolding({
                        ...
                        name:
                            newName.trim() || data.name || authoritativeSymbol,
                        shares: sharesNum,
                        price: data.price,
                ...
                onUpdateHolding(cashHolding.id, {
                    ...
                    cost:
                        (cashHolding.cost || cashHolding.value) - addedCost > 0
                            ? (cashHolding.cost || cashHolding.value) -
                              addedCost
                            : remainingValue,
                });
            }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-4">
                        <div className="flex flex-col gap-1">
                            ...
                            <select
                                value={newAssetId}
                                onChange={(e) =>
                                    setNewAssetId(e.target.value as AssetId)
                                }
                    ...
                    <tbody className="divide-y divide-[#F8F9FA] bg-white text-[#1A1A1A]">
                        {holdings.length === 0 ? (
                            <tr>
                                <td
                                    colSpan={6}
                                    className="px-6 py-12 text-center text-sm text-[#ADB5BD]"
                                >
                ...
                <div className="fixed inset-0 bg-[#1A1A1A]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl flex flex-col gap-6">
                        ...
                        <div className="space-y-4">
                            {sellingHolding.shares &&
                            sellingHolding.shares > 0 ? (
                                <>
