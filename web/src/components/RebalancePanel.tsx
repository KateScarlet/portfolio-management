    ...
    <div className="bg-white rounded-2xl border border-[#E9ECEF] shadow-sm flex flex-col overflow-hidden h-full">
      ...
      <div className="p-6 bg-white space-y-4">
        {allBalanced && (
          <div className="p-4 bg-white border border-[#E9ECEF] rounded-lg flex items-center gap-4 mb-2">
            <div className="w-10 h-10 rounded-full bg-[#F8F9FA] flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-[#1A1A1A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {items.map((item) => (
            <div key={item.id} className="p-4 rounded-xl border border-[#E9ECEF] hover:bg-[#F8F9FA] transition-colors flex flex-col justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-[#ADB5BD] mt-0.5 flex flex-wrap items-center gap-2">
                    ...
                    <svg className="w-3 h-3 text-[#ADB5BD]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    <span className="font-mono font-medium text-[#6C757D]">{formatCurrency(item.targetValue)}</span>
                  </div>
                </div>
              </div>

              <div className={`flex items-center justify-center gap-2 px-4 py-2 rounded-full font-medium text-[11px] w-full border tracking-wide uppercase
                ${item.action === 'buy' ? 'bg-[#1A1A1A] text-white border-[#1A1A1A]' :
                  item.action === 'sell' ? 'bg-white text-orange-600 border-orange-200' :
                    'bg-[#F8F9FA] text-[#6C757D] border-[#E9ECEF]'}`
              }>
                <span>
                  {item.action === 'buy' ? '补仓 · ' :
                   item.action === 'sell' ? '减仓 · ' : '保持 · '}
